...
export function AdminShell({ activeKey, title, subtitle, navigateTo, children, primaryAction }) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      Alert.alert('تعذر تسجيل الخروج', error?.message || 'حدث خطأ غير متوقع');
    }
  };
        ...
        <View style={[styles.topbar, isMobile && styles.topbarMobile]}>
          <View style={[styles.topbarHead, isMobile && styles.topbarHeadMobile]}>
            ...
            <View style={[styles.topbarText, isMobile && styles.topbarTextMobile]}>
              <Text style={[styles.pageTitle, isMobile && styles.pageTitleMobile]} numberOfLines={2} ellipsizeMode="tail">{title}</Text>
              <Text style={[styles.pageSubtitle, isMobile && styles.pageSubtitleMobile]} numberOfLines={2} ellipsizeMode="tail">{subtitle}</Text>
            </View>
          </View>
          <View style={[styles.topbarActions, isMobile && styles.topbarActionsMobile]}>
            ...
            <TouchableOpacity style={[styles.iconButton, isMobile && styles.iconButtonMobile]} onPress={() => Alert.alert('الإشعارات', 'لا توجد إشعارات جديدة')}>
            ...
            >
              {visibleColumns.map((column) => (
                <View key={column.key} style={styles.mobileRecordRow}>
                  <Text style={styles.mobileRecordLabel} numberOfLines={1}>{column.label}</Text>
...
export function StatusBadge({ status }) {
  ...
  return (
    <View style={[styles.statusBadge, { backgroundColor }]}>
      <Text style={[styles.statusText, { color }]}>{status || 'نشط'}</Text>
    </View>
  );
}
...
export function ErrorState({ message }) {
  return (
    <View style={styles.stateBox}>
      ...
      <Text style={styles.stateText}>{message || 'تعذر تحميل البيانات'}</Text>
    </View>
  );
}
...
export function FormInput({ label, value, onChangeText, multiline, ...props }) {
  return (
...
const styles = StyleSheet.create({
  shell: { flex: 1, flexDirection: 'row-reverse', backgroundColor: adminColors.bg },
  ...
  topbarHeadMobile: { flex: 0, width: '100%', alignItems: 'flex-start' },
  topbarText: { flex: 1, minWidth: 0, alignItems: 'flex-end' },
  topbarTextMobile: { flexShrink: 1 },
  ...
  menuButton: { width: 42, height: 42, borderRadius: 14, borderWidth: 1, borderColor: adminColors.border, backgroundColor: adminColors.softBlue, alignItems: 'center', justifyContent: 'center' },
  ...
  drawerLayer: { flex: 1, flexDirection: 'row-reverse' },
  drawerBackdrop: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(16,42,104,0.35)' },
  mobileDrawer: { height: '100%', backgroundColor: adminColors.primary, borderTopLeftRadius: 24, borderBottomLeftRadius: 24, overflow: 'hidden', ...adminShadow },
  ...
  statCardMobile: { width: '100%', flexBasis: '100%', flexGrow: 0, minHeight: 94 },
