...
const MOTION_VISUAL_MULTIPLIER = 1.35;
const MOTION_VISUAL_MIN_MS = 1400;
const FULL_RULER_TRAVEL_MS = 3200;
  ...
  if (level?.id === 'physics_lab_level_2') {
    return {
      ...
      items: [
        ...
        `Current force: ${params.force} N`,
        `Current friction: ${params.friction}`,
        `Net acceleration now: ${currentMeta.netAcceleration ?? 'calculated after run'} m/s^2`,
      ],
    };
  }
...
function buildTaskPrompt(level) {
  ...
  if (level?.id === 'physics_lab_level_2') {
    return {
      title: 'Question',
      body: 'Choose a force and friction value that stop the box at the target.',
      hint: 'Balance the push against the resisting force.',
    };
  }
...
export default function PhysicsLabLevelScreen({ route, navigation }) {
  ...
  async function handleRun() {
    ...
    try {
      ...
      const visualMotionDurationMs = level?.id === 'physics_lab_level_1'
        ? Math.max(900, Math.round(motionDurationMs))
        : Math.max(
  ...
  function handleRetry() {
    if (running || canLeaveAsWinner) return;
  ...
  return (
    <ScreenContainer scroll>
      ...
      <View style={[styles.mainRow, showSideRuleBook && styles.mainRowWide]}>
        <View style={[styles.primaryColumn, showSideRuleBook && styles.primaryColumnWide]}>
          <View style={styles.taskCard}>
            <Text style={styles.taskKicker}>{taskPrompt.title}</Text>
            <Text style={styles.taskTitle}>{taskPrompt.body}</Text>
            <Text style={styles.taskHint}>{taskPrompt.hint}</Text>
          </View>
          ...
          <PhysicsControlPanel
            ...
            mainActionDisabled={running}
            mainActionLabel={canLeaveAsWinner ? 'Back to Levels' : running ? 'Running...' : undefined}
          />
        ...
        <View style={[styles.sideColumn, showSideRuleBook ? styles.sideColumnVisible : styles.sideColumnStacked]}>
          <PhysicsRuleBook teaching={level.teaching} compact={!showSideRuleBook} />
        </View>
      </View>
